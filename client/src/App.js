...
const App = () => {
  ...
  useEffect(() => {
    axios.get('https://todorepo-c68n.onrender.com/gettasks').then((arr) => setItem(arr.data));
  }, []);

  const submitHandler = (e) => {
    e.preventDefault();
    axios.post('https://todorepo-c68n.onrender.com/addtask', { todo: newTask }).then((arr) => setItem(arr.data));
    setNewtask('');
  };

  const deleteHandler = (id) => {
    axios.delete(`https://todorepo-c68n.onrender.com/delete/${id}`).then((arr) => setItem(arr.data));
  };
